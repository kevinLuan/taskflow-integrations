...
import cn.feiliu.taskflow.common.utils.EnvUtils.SystemParameters;
...
public class ConstraintParamUtil {
     ...
     */
    public static List<String> validateInputParam(Map<String, Object> input, String taskName,
            ...
            if (s.startsWith("${") && s.endsWith("}")) {
                ...
                if (StringUtils.containsWhitespace(paramPath)) {
                    ...
                    if (!isPredefinedEnum) {
                        ...
                        if (sysValue == null) {
                            String errorMessage = String.format("environment variable: %s for given task: %s"
                                                                + " input value: %s"
                                                                + " of input parameter: %s is not valid", paramPath,
                                taskName, key, value);
                    ...
                    if (!"workflow".equals(components[0])) {
                        FlowTask task = workflow.getTaskByRefName(components[0]);
                        if (task == null) {
                            String message = String.format(
                                "taskReferenceName: %s for given task: %s input value: %s of input" + " parameter: %s"
                                        + " is not defined in workflow definition.", components[0], taskName, key,
                                value);
                            errorList.add(message);
                        }
                    }
