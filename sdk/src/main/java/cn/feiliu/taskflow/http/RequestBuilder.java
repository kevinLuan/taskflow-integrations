...
import cn.feiliu.taskflow.http.ProgressRequestBody.ProgressRequestListener;
...
public class RequestBuilder {
    ...
    public RequestBuilder queryParam(String name, Object value) {
        if (value != null) {
            this.queryParams.addAll(ClientHelper.parameterToPair(name, value));
        }
        return this;
    }
    ...
    public Request build() throws ApiException {
        ...
        final String url = ClientHelper.buildUrl(client, path, queryParams, collectionQueryParams);
        final Request.Builder reqBuilder = new Request.Builder().url(url);
        ClientHelper.processHeaderParams(client, headerParams, reqBuilder);
        String contentType = Optional.ofNullable(headerParams.get("Content-Type")).orElse("application/json");
        RequestBody reqBody;
        if (!HttpMethod.permitsRequestBody(method)) {
            reqBody = null;
        } else if ("application/x-www-form-urlencoded".equals(contentType)) {
            reqBody = ClientHelper.buildRequestBodyFormEncoding(formParams);
        } else if ("multipart/form-data".equals(contentType)) {
            reqBody = ClientHelper.buildRequestBodyMultipart(formParams);
        } else if (body == null) {
            ...
            reqBody = ClientHelper.serialize(body, contentType);
        }
