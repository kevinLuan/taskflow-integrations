...
public class FieldUtils {
    ...
    private static boolean hasPublicGetter(Class<?> type, Field field) {
        ...
        // 可能的 Getter 方法名前缀
        String[] prefixes = field.getType() == boolean.class || field.getType() == Boolean.class
                ? new String[]{"is", "get"}
                : new String[]{"get"};
    ...
    private static boolean isCompatibleReturnType(Method getter, Field field) {
        ...
        if (fieldType.isPrimitive()) {
            if (fieldType == boolean.class && returnType == Boolean.class) return true;
            if (fieldType == int.class && returnType == Integer.class) return true;
            if (fieldType == double.class && returnType == Double.class) return true;
            if (fieldType == float.class && returnType == Float.class) return true;
            if (fieldType == long.class && returnType == Long.class) return true;
            if (fieldType == short.class && returnType == Short.class) return true;
            if (fieldType == byte.class && returnType == Byte.class) return true;
            if (fieldType == char.class && returnType == Character.class) return true;
            if (fieldType == void.class && returnType == Void.class) return true;
        }
